...
export default function LoginPage() {
  ...
  const fillDemo = () => {
    setForm({ email: "faizan@test.com", password: "123456" });
    setError("");
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.token) {
        ...
        saveUser({ name: data.name, email: data.email, id: data._id });
        setSuccess(true);
  ...
  const inputStyle = (field) => ({
    ...
    border: `1.5px solid ${
      error && !form[field] ? "#EF4444" : focused === field ? "#C0182B" : "#E5E7EB"
    }`,
    borderRadius: 10,
    fontSize: 15,
    color: "#1F2937",
    ...
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#FFF5F5", fontFamily: "Inter, sans-serif" }}>
        <div style={{ textAlign: "center", padding: 48 }}>
          ...
          <h2 style={{ fontSize: 26, fontWeight: 800, color: "#1F2937", margin: "0 0 8px" }}>Welcome back!</h2>
          <p style={{ color: "#6B7280", marginBottom: 28, fontSize: 15 }}>Redirecting you to your dashboard…</p>
          <button
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", fontFamily: "Inter, sans-serif", overflow: "hidden" }}>
      ...
      {/* ── LEFT PANEL ── */}
      <div style={{ width: "40%", minWidth: 320, background: "linear-gradient(160deg, #8B0000 0%, #C0182B 55%, #E53E3E 100%)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "60px 40px", position: "relative", overflow: "hidden" }}>
        {[180, 300, 420, 540].map((size, i) => (
          <div key={i} style={{ position: "absolute", width: size, height: size, borderRadius: "50%", border: "1px solid rgba(255,255,255,0.06)", top: "50%", left: "50%", transform: "translate(-50%, -50%)" }} />
        ))}
        ...
        <div style={{ position: "relative", zIndex: 1, textAlign: "center" }}>
          <div style={{ width: 110, height: 110, borderRadius: "50%", background: "rgba(255,255,255,0.12)", border: "2px solid rgba(255,255,255,0.22)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 28px", boxShadow: "0 0 50px rgba(255,255,255,0.1), 0 0 100px rgba(192,24,43,0.3)", animation: "heartbeat 1.6s ease-in-out infinite" }}>
            <HeartbeatIcon size={56} color="white" />
          </div>
          <h1 style={{ fontSize: 32, fontWeight: 900, color: "white", margin: "0 0 12px", letterSpacing: "-0.6px", lineHeight: 1.2 }}>Welcome Back</h1>
          <p style={{ fontSize: 15, color: "rgba(255,255,255,0.7)", lineHeight: 1.7, maxWidth: 240, margin: "0 auto 40px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {[
              ...
              <div key={stat.value} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 10, padding: "10px 16px" }}>
        ...
        <div style={{ position: "absolute", bottom: 24, display: "flex", alignItems: "center", gap: 7 }}>
          <HeartbeatIcon size={18} color="white" />
          <span style={{ fontWeight: 800, fontSize: 14, color: "rgba(255,255,255,0.65)", letterSpacing: "-0.2px" }}>HeartGuard</span>
        </div>
      </div>

      {/* ── RIGHT PANEL ── */}
      <div style={{ flex: 1, background: "white", display: "flex", alignItems: "center", justifyContent: "center", padding: "48px 64px" }}>
        <div style={{ width: "100%", maxWidth: 420 }}>

          {/* Header */}
          <div style={{ marginBottom: 36 }}>
            <h2 style={{ fontSize: 30, fontWeight: 900, color: "#1F2937", margin: "0 0 8px", letterSpacing: "-0.8px" }}>
              Sign in to HeartGuard
            </h2>
            <p style={{ fontSize: 15, color: "#6B7280", margin: 0 }}>
              Don't have an account?{" "}
              <a href="/register" style={{ color: "#C0182B", fontWeight: 600, textDecoration: "none" }}
          ...
          <div
            onClick={fillDemo}
            style={{ display: "flex", alignItems: "center", gap: 10, background: "#F0FDF4", border: "1.5px dashed #86EFAC", borderRadius: 10, padding: "11px 14px", marginBottom: 24, cursor: "pointer", transition: "all 0.2s" }}
          ...
          >
            <ShieldCheckIcon />
            <div>
              <p style={{ fontSize: 12, fontWeight: 700, color: "#15803D", margin: 0 }}>Test Account — click to autofill</p>
              <p style={{ fontSize: 11.5, color: "#4ADE80", margin: 0, fontWeight: 500 }}>faizan@test.com · 123456</p>
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 7 }}>
                ...
                <a href="#" style={{ fontSize: 13, color: "#C0182B", fontWeight: 600, textDecoration: "none" }}
                  onMouseEnter={e => e.target.style.textDecoration = "underline"}
                  onMouseLeave={e => e.target.style.textDecoration = "none"}>
                  Forgot password?
                </a>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
              <div
                ...
                style={{ width: 18, height: 18, borderRadius: 5, border: `2px solid ${rememberMe ? "#C0182B" : "#D1D5DB"}`, background: rememberMe ? "#C0182B" : "white", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", flexShrink: 0, transition: "all 0.2s" }}
              >
                {rememberMe && (
              ...
              <span style={{ fontSize: 13, color: "#6B7280", fontWeight: 500 }}>Remember me for 30 days</span>
            </div>
            ...
            <button
              ...
              disabled={loading}
              style={{ width: "100%", padding: "14px", background: loading ? "#E5E7EB" : "linear-gradient(135deg, #C0182B 0%, #8B0000 100%)", color: loading ? "#9CA3AF" : "white", border: "none", borderRadius: 10, fontWeight: 700, fontSize: 15, cursor: loading ? "not-allowed" : "pointer", transition: "all 0.25s", boxShadow: loading ? "none" : "0 4px 16px rgba(192,24,43,0.3)", fontFamily: "Inter, sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 4, letterSpacing: "0.01em" }}
              onMouseEnter={e => { if (!loading) { e.currentTarget.style.transform = "translateY(-1px)"; e.currentTarget.style.boxShadow = "0 6px 24px rgba(192,24,43,0.4)"; }}}
              onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.boxShadow = loading ? "none" : "0 4px 16px rgba(192,24,43,0.3)"; }}
            >
              {loading ? (
                <>
          ...
          <p style={{ fontSize: 11.5, color: "#9CA3AF", textAlign: "center", marginTop: 28, lineHeight: 1.6 }}>
