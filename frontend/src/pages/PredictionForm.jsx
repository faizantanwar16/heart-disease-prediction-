  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
      <svg width="220" height="130" viewBox="0 0 220 130">
        <path d={`M ${cx - R} ${cy} A ${R} ${R} 0 0 1 ${cx + R} ${cy}`} fill="none" stroke="#F3F4F6" strokeWidth="16" strokeLinecap="round"/>
      ...
      <div style={{ textAlign: "center", marginTop: -8 }}>
        ...
        <div style={{ display: "inline-flex", alignItems: "center", gap: 6, background: riskBg, border: `1.5px solid ${riskColor}40`, borderRadius: 20, padding: "5px 14px", marginTop: 8 }}>
...
const labelStyle = { fontSize: 12, fontWeight: 700, color: "#374151", display: "block", marginBottom: 5, letterSpacing: "0.01em" };
const descStyle  = { fontSize: 11, color: "#9CA3AF", marginTop: 4, display: "flex", alignItems: "center", gap: 4 };
    ...
    {[{ val: "1", label: yesLabel }, { val: "0", label: noLabel }].map(({ val, label }) => {
      ...
      return (
        <button key={val} type="button" onClick={() => onChange(val)}
          style={{ flex: 1, padding: "9px 0", borderRadius: 9, border: `1.5px solid ${active ? "#C0182B" : "#E5E7EB"}`, background: active ? "#FFF5F5" : "white", color: active ? "#C0182B" : "#6B7280", fontWeight: active ? 700 : 500, fontSize: 13, cursor: "pointer", transition: "all 0.18s", fontFamily: "Inter, sans-serif" }}>
          {label}
        </button>
      );
    })}
...
const NumberInput = ({ value, onChange, min, max, step = 1, placeholder, focused, onFocus, onBlur, unit }) => (
  <div style={{ position: "relative" }}>
    <input type="number" value={value} onChange={(e) => onChange(e.target.value)} onFocus={onFocus} onBlur={onBlur}
...
const FieldWrapper = ({ label, desc, children }) => (
  <div>
    ...
    {children}
    {desc && <p style={descStyle}><InfoIcon />{desc}</p>}
  </div>
);
      ...
      <div style={{ marginLeft: 240, flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ padding: "24px 28px", flex: 1, display: "flex", gap: 24, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 20, minWidth: 0 }}>
                    ...
                    {bmi ? (
                      <>
                        ...
                        <span style={{ fontSize: 12, color: "#6B7280" }}>
                          kg/m² — <span style={{ fontWeight: 600, color: parseFloat(bmi) >= 30 ? "#C0182B" : parseFloat(bmi) >= 25 ? "#D97706" : "#16A34A" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                <div style={{ gridColumn: "1 / -1" }}>
                  <FieldWrapper label="#11 — Diabetic Status" desc="Diabetic status (4 categories)">
                    <SelectField value={form.diabetic} onChange={set("diabetic")} placeholder="Select diabetic status…"
            ...
            <div style={{ background: "white", borderRadius: 16, padding: "24px", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", border: "1px solid #F3F4F6" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                <FieldWrapper label="#9 — Age Category" desc="Age group in 5-year bands">
                  <SelectField value={form.ageCategory} onChange={set("ageCategory")} placeholder="Select age group…"
                ...
                <FieldWrapper label="#10 — Race" desc="Racial/ethnic background">
                  <SelectField value={form.race} onChange={set("race")} placeholder="Select race…"
                ...
                <FieldWrapper label="#13 — General Health" desc="Self-rated general health">
                  <SelectField value={form.genHealth} onChange={set("genHealth")} placeholder="Rate your health…"
            ...
            {result !== null && (
              <button onClick={handleReset}
                style={{ width: "100%", padding: "11px", background: "transparent", border: "1.5px solid #E5E7EB", borderRadius: 12, fontWeight: 600, fontSize: 14, color: "#6B7280", cursor: "pointer", fontFamily: "Inter, sans-serif", transition: "all 0.2s" }}
          ...
          <div style={{ width: 320, flexShrink: 0, display: "flex", flexDirection: "column", gap: 18, position: "sticky", top: 88 }}>
            ...
            {/* Progress card */}
            <div style={{ background: "white", borderRadius: 16, padding: "18px", boxShadow: "0 2px 12px rgba(0,0,0,0.05)", border: "1px solid #F3F4F6" }}>
              ...
              <p style={{ margin: 0, fontSize: 12, color: "#9CA3AF" }}>
                {filledCount} of {total} features entered{pct === 100 && " ✓ Ready to predict!"}
              </p>
            </div>
